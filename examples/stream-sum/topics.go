...
type Chunk struct {
	...
	Data  []byte   // Raw data included in the Value. If empty, this is the streams final chunk.
}
...
// ReadPRNG generates fast but high-quality random entropy into the provided byte slice.
...
var prngSource = sync.Pool{
	New: func() interface{} {
		...
		if aesCipher, err := aes.NewCipher(key[:]); err != nil {
			panic(err) // Should never error (give correct |key| size).
		} else {
