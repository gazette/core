...
`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		cfs, err := cloudstore.NewFileSystem(nil, viper.GetString("cloud.fs.url"))
		if err != nil {
			...
			for i := 0; i != concurrentDeletes; i++ {
				...
				go func() {
					...
					for f := range deleteCh {
						if err := cfs.Remove(f.ContentPath()); err != nil {
		...
		if err = cfs.Walk(hints.Log.String(), journal.NewWalkFuncAdapter(func(f journal.Fragment) error {
			nTotal += 1
			bytesTotal += f.Size()
