...
func NewGazetteServiceEndpoint() *string {
	return CommandLine.ServiceEndpoint("gazette", "127.0.0.1:8081", "Gazette network service host:port")
}

// NewEtcdServiceEndpoint defines the Etcd service endpoint flag.
func NewEtcdServiceEndpoint() *string {
	return CommandLine.ServiceEndpoint("etcd", "127.0.0.1:2379", "Etcd network service host:port")
}

// NewCloudFSURL defines the cloudFS URL flag.
func NewCloudFSURL() *string {
	return CommandLine.String("cloudFS", "CLOUD_FS_URL", "file:///cloud-fs", "URL parameterizing the cloud filesystem to use.")
}
