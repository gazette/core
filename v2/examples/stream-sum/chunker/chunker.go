...
func (runChunker) Execute(args []string) error {
	...
	var ajc = client.NewAsyncJournalClient(rjc, client.NewAppendService(ctx, rjc))
	...
	var barrier = ajc.StartAppend(stream_sum.SumsJournal)
	barrier.Release()
	<-barrier.Done()
	...
	verify(func(chunk stream_sum.Chunk) {
		if _, err := message.Publish(ajc, chunksMapping, &chunk); err != nil {
