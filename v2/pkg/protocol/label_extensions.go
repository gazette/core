...
func (m LabelSelector) Matches(s LabelSet) bool {
	if any, _ := matchSelector(m.Exclude.Labels, s.Labels); any {
		return false
	} else if _, all := matchSelector(m.Include.Labels, s.Labels); !all {
		return false
	}
	return true
}
...
func matchSelector(sel, set []Label) (any bool, all bool) {
	all = true
	...
	for cur, ok := it.next(); ok; cur, ok = it.next() {
		if cur.lBeg == cur.lEnd {
			continue // |set| label which is not in |sel|.
		}
		...
		for a, b := sel[cur.lBeg:cur.lEnd], set[cur.rBeg:cur.rEnd]; !matched && len(a) != 0 && len(b) != 0; {
			if a[0].Value == "" || a[0].Value == b[0].Value {
				matched = true // Selector value "" matches any value of the label.
...
//
// ParseLabelSelector is invariant to most whitespace. "not in" and "notin"
// may be used interchangeably, as may "==" and "=".
