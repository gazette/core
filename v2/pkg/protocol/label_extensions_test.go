...
func (s *LabelSuite) TestSetValidationCases(c *gc.C) {
	...
	c.Check(set.Validate(), gc.ErrorMatches, `Labels\[0\].Name: not base64 alphabet \(bad label\)`)
	...
	c.Check(set.Validate(), gc.ErrorMatches, `Label has empty & non-empty values \(index 5; label ccc value 555\)`)
	set.Labels[4].Value = "55"
