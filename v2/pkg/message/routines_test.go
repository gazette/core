...
func (s *RoutinesSuite) TestPublishSuccess(c *gc.C) {
	...
	var ajc = client.NewAsyncJournalClient(rjc, client.NewAppendService(context.Background(), rjc))

	var mapping = func(msg Message) (pb.Journal, Framing, error) {
		return "a/journal", JSONFraming, nil
	}
	var aa, err = Publish(ajc, mapping, struct{ Data string }{Data: "beer beer beer"})
