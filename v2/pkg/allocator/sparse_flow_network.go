...
import (
	...
	pr "github.com/LiveRamp/gazette/v2/pkg/allocator/sparse_push_relabel"
...
const (
	// By network construction, we'll relabel an Item node to a RelativeHeight
	// of 1 just prior to pushing back to the source (terminating push/relabel).
	// If we reach this height, allow a single zone to hold all item replicas
	// as the alternative is not fully replicating the item at all.
	...
	// take on more than its fair share of Items then cause an Item to not be
	...
	// skipping S, so we must bound to RelativeHeight -1 to ensure this never
	// happens.
...
func (fs *sparseFlowNetwork) Arcs(mf *pr.MaxFlow, id pr.NodeID, page pr.PageToken) ([]pr.Arc, pr.PageToken) {
	if id == pr.SourceID {
		...
		// an assignment to any of the zone member.
		switch page {
		case pr.PageInitial:
