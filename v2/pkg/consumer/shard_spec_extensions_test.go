...
func (s *SpecSuite) TestShardSpecValidationCases(c *gc.C) {
	...
	c.Check(spec.Validate(), gc.ErrorMatches, `Id: not base64 alphabet \(bad id\)`)
	spec.Id = "a-shard-id"
	...
	c.Check(spec.Validate(), gc.ErrorMatches, `RecoveryLog: not base64 alphabet \(bad log\)`)
	...
	c.Check(spec.Validate(), gc.ErrorMatches, `LabelSet.Labels\[0\].Name: not base64 alphabet \(bad label\)`)
	spec.Labels[0].Name = "label"

	c.Check(spec.Validate(), gc.ErrorMatches, `Sources\[0\].Journal: not base64 alphabet \(journal 2\)`)
...
func (s *SpecSuite) TestConsumerSpecValidationCases(c *gc.C) {
	...
	c.Check(spec.Validate(), gc.ErrorMatches, `Id.Zone: not base64 alphabet \(not valid\)`)
	spec.Id.Zone = "zone"
...
func (s *SpecSuite) TestListResponseValidationCases(c *gc.C) {
	...
	c.Check(resp.Validate(), gc.ErrorMatches, `Shards\[0\].Spec.Id: not base64 alphabet .*`)
...
func (s *SpecSuite) TestApplyRequestValidationCases(c *gc.C) {
	...
	c.Check(req.Validate(), gc.ErrorMatches, `Changes\[0\].Upsert.Id: not base64 alphabet \(invalid id\)`)
	...
	c.Check(req.Validate(), gc.ErrorMatches, `Changes\[1\].Delete: not base64 alphabet \(another invalid id\)`)
