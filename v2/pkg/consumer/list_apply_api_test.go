...
func (s *ListApplySuite) TestListCases(c *gc.C) {
	...
	c.Check(err, gc.ErrorMatches, `Selector.Include.Labels\[0\].Name: not base64 alphabet \(invalid label\)`)
...
func (s *ListApplySuite) TestApplyCases(c *gc.C) {
	...
	c.Check(err, gc.ErrorMatches, `Changes\[0\].Delete: not base64 alphabet \(invalid shard id\)`)
}
