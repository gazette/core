...
// PolledList performs periodic polls of a ListRequest. Its must recent
...
func (pl *PolledList) periodicRefresh(dur time.Duration) {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				log.WithFields(log.Fields{"err": err, "req": pl.req.String()}).Warn("period List refresh failed")
			} else {
				pl.resp.Store(resp)
			}
		case <-pl.ctx.Done():
