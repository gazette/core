...
// NewAsyncJournalClient returns a composed JournalClient and AsyncAppendClient.
func NewAsyncJournalClient(rjc pb.RoutedJournalClient, as *AppendService) AsyncJournalClient {
	return struct {
		pb.RoutedJournalClient
		*AppendService
	}{rjc, as}
...
func (s *AppendService) StartAppend(name pb.Journal, dependencies ...*AsyncAppend) *AsyncAppend {
	...
	// *before* we skip forward to the current AsyncAppend. This ensure
...
type AsyncAppend struct {
	...
	checkpoint   int64          // Buffer offset to append through, or -1 if in an initial state.
