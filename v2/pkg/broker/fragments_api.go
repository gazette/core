...
func buildSignedFragments(req *pb.FragmentsRequest, fragmentSet fragment.CoverSet) ([]pb.FragmentsResponse_SignedFragment, int64, error) {
	var signedFragments = make([]pb.FragmentsResponse_SignedFragment, 0, req.PageLimit)
	var ind = sort.Search(len(fragmentSet), func(i int) bool {
		return fragmentSet[i].Begin >= req.NextPageToken
	})
	var found = ind < len(fragmentSet) && fragmentSet[ind].Begin <= req.NextPageToken
	...
	for i, f = range fragmentSet[ind:] {
		if len(signedFragments) == cap(signedFragments) {
			break
		}
		...
		if req.EndModTime.IsZero() && f.BackingStore == "" {
			signedFragments = append(signedFragments, pb.FragmentsResponse_SignedFragment{Fragment: f.Fragment})
			continue
		}
		...
		if (req.BeginModTime.Equal(f.ModTime) || req.BeginModTime.Before(f.ModTime)) &&
			...
			signedFragments = append(signedFragments, signedFragment)
		}
	}

	var nextIndex = i + ind
	if len(signedFragments) > 0 && nextIndex < len(fragmentSet)-1 {
		return signedFragments, fragmentSet[nextIndex].Begin, nil
	}
	return signedFragments, 0, nil
}
