...
func checkHealth(res resolution, jc pb.JournalClient, etcd clientv3.KV) (int64, error) {
	...
	var proposal = checkFragmentFlush(pln.spool, res.journalSpec.Fragment)
...
func checkFragmentFlush(cur fragment.Spool, spec pb.JournalSpec_Fragment) pb.Fragment {
	...
	if cur.ContentLength() == 0 || cur.ContentLength() > spec.Length {
		return getFragmentFlushProposal(cur.Fragment.Fragment, spec)
	}
