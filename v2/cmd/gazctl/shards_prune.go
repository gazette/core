...
func (cmd *cmdShardsPrune) Execute([]string) error {
	...
	for _, shard := range listShards(cmd.Selector).Shards {
		...
		if lastHints == nil {
			log.Infof("skipping shard %s, there have are no backup hints for this shard", shard.Spec.Id)
			continue
		}
		...
		for _, sf := range fetchFragments(ctx, lastHints.Log) {
			...
			if len(segments.Intersect(sf.Begin, sf.End)) == 0 {
				...
				if !cmd.DryRun {
					...
					if err != nil {
						mbp.Must(err, fmt.Sprintf("error removing fragment %v", sf.BackingStore.URL().String()+sf.ContentName()))
					}
