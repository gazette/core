...
// If `updateCh` is non-nil, then it is signaled upon the first and all subsequent
// snapshot updates and returned via UpdateCh(). This is recommended if you have a
// dynamic set of WatchedList instances and which to take action if any one of them update.
// Otherwise, when `updateCh` is nil then a new channel with a single buffered item is allocated.
