...
func (m *JournalSpec_Suspend) Validate() error {
	if m == nil {
		return nil
	} else if err := m.Level.Validate(); err != nil {
		return ExtendContext(err, "Level")
	} else if m.Offset < 0 {
		return ExtendContext(NewValidationError("invalid Offset (%d; expected >= 0)", m.Offset), "Offset")
	}
	return nil
}
...
func (m *JournalSpec) DesiredReplication() int {
	...
	if m.Suspend.Level == JournalSpec_Suspend_PARTIAL {
		r = 1 // Journal is suspended down to a single read-only replica.
	} else if m.Suspend.Level == JournalSpec_Suspend_FULL {
