...
func (s *DatabaseSuite) TestDatabase(c *gc.C) {
	...
	var promise = make(async.Promise)
	promise.Resolve()

	var writer = &journal.MockWriter{}
	writer.On("Write", logName, mock.AnythingOfType("[]uint8")).Return(promise, nil)
	writer.On("ReadFrom", logName, mock.Anything).Return(promise, nil)
	...
	// Commit. Expect |promise| is passed through as a write barrier,
	// and that writeBatch was flushed.
	barrier, err := db.commit()
	c.Check(db.writeBatch.Count(), gc.Equals, 0)
	c.Check(barrier, gc.Equals, (<-chan struct{})(promise))
