...
The revision of specs are always compared to the current server revision,
an apply will fail if there's a mismatch. This prevents an edit or
get/update/apply operation from overwriting specification changes which may
...
revision should be ignored. This is helpful when the desired source-of-truth
for a set of JournalSpecs is versioned source control, where an application
of those specs should always overwrite any existing versions.
...
``gazctl journals edit`` workflow. A ``delete`` stanza set on a parent node also applies
to all children.

.. code-block:: yaml

    name: examples/foobar
    delete: true # Added by user.
    revision: 6
    replication: 3
    fragment:
        ... etc ...
