...
a re-send of a prior adopted Fragment.
...
the journal primary broker employs a
...
[somewhat tight timeout](../broker/append_fsm.go#L17)
on the delivery of streamed client chunks. Clients are expected to be able to quickly
produce all content chunks. When operating over lossy networks or with untrusted
clients, it likely makes sense to having a buffering proxy that's closer to the broker.
